...
// tier uses. CPU% is the WHOLE VM (every core, every process) read from /proc/stat; memory is the
// resident working set of the app processes (this Node tier + the spawned rindled), NOT page cache,
// so it reads as the honest "tiny footprint" rather than drifting toward the limit as the kernel
// caches the sqlite file. Off-Linux (a dev mac) /proc is absent and the numbers degrade gracefully
// (cpuPercent stays null; memory falls back to this process's own RSS).
