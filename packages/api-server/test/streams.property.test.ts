...
// (designs/LM-STREAM-CHECKPOINT-DESIGN.md §0, §9).
//
...
const opArb: fc.Arbitrary<Op> = fc.oneof(
  ...
  { weight: 2, arbitrary: fc.integer({ min: 0, max: 3 }).map((fromKind): Op => ({ t: "subscribe", fromKind })) },
...
class SubModel {
  ...
  private check(frame: StreamFrame): void {
    ...
    switch (frame.type) {
      ...
      case "chunk":
        ...
        assert.ok(produced.startsWith(this.acc + frame.text), "a chunk carries the produced text verbatim");
        this.acc += frame.text;
        break;
      case "durable":
  ...
  finish(): void {
    const produced = this.producedRef();
    if (this.terminal === "end") {
      assert.equal(this.acc, produced, "S: join prefix ++ chunks == produced, for every join offset and time");
    } else {
      // Dropped (stale), evicted (absent), or closed early: what it saw must still be a prefix.
