...
mod build_bundled {
    ...
    pub fn main(out_dir: &str, out_path: &Path) {
        ...
        cfg.file(format!("{lib_name}/sqlite3.c"))
            ...
            // Enables sqlite3_stmt_scanstatus_v2 (the planner's per-loop row
            // estimates), used by the rindle-sqlite cost model (planner Stage 8).
        ...
        // VENDORED CHANGE (rindle-replica): compile the preupdate hook in
        // UNCONDITIONALLY. The prebuilt bindings (`bindgen_bundled_version.rs`)
        // already DECLARE sqlite3_preupdate_hook/_count/_old/_new, so only the C
        // *definitions* are missing — gating this behind the `preupdate_hook` cargo
        // feature would pull `buildtime_bindgen` → libclang and destroy the hermetic
        // prebuilt-bindings build. Defining the symbols here makes them linkable with
        // no libclang; `rindle-replica`'s CDC layer installs the hook via raw FFI.
